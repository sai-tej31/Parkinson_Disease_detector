...
for p in range (0,len(healthy)):
    ...
    for i in range (0,len(array)):
        if int(array[i].split(';')[6])==0:
            ...
            time_s = ((tstamp1 - tstamp2).total_seconds() / 60)
            if time_s < 0:
                time_s = time_s *(-1)
            ...
            tstamp1 = datetime.strptime(str(dt_object), fmt)
            tstamp2 = datetime.strptime(str(dt_object1), fmt)
            time_d = (tstamp1 - tstamp2).total_seconds() / 60
            if time_d < 0:
                time_d = time_d *(-1)
        elif int(array[i].split(';')[6])==2:
            pc.append(int(array[i].split(';')[3]))
            gac.append(int(array[i].split(';')[4]))
            dt_object = datetime.fromtimestamp(int(array[-1].split(';')[5]))
            dt_object1 = datetime.fromtimestamp(int(array[0].split(';')[5]))
            tstamp1 = datetime.strptime(str(dt_object), fmt)
            tstamp2 = datetime.strptime(str(dt_object1), fmt)
            time_c = ((tstamp1 - tstamp2).total_seconds() / 60)
            if time_c < 0:
                time_c = time_c *(-1)
    avg.append({'pressure' : np.average(ps),'grip_angle':np.average(gas),'timestamp': time_s,'state': 'static' ,'status' : 1 })
    avg.append({'pressure' : np.average(pd),'grip_angle':np.average(gad),'timestamp': time_d,'state': 'dynamic' ,'status' : 1 })
    avg.append({'pressure' : np.average(pc),'grip_angle':np.average(gac),'timestamp': time_c,'state': 'circular' ,'status' : 1 })
...
for p in range (0,len(parkinson)):
    ...
    for i in range (0,len(array)):
        if int(array[i].split(';')[6])==0:
            ...
            if time_c < 0:
                time_c = time_c *(-1)
    avg.append({'pressure' : np.average(ps),'grip_angle':np.average(gas),'timestamp': time_s,'state': 'static' ,'status' : 0 })
    avg.append({'pressure' : np.average(pd),'grip_angle':np.average(gad),'timestamp': time_d,'state': 'dynamic' ,'status' : 0 })
    avg.append({'pressure' : np.average(pc),'grip_angle':np.average(gac),'timestamp': time_c,'state': 'circular' ,'status' : 0 })
